...
const InventoryDetails = () => {
  ...
  useEffect(() => {
    setLoading(true);
    fetch(`https://inventory-update-server.vercel.app/inventory/${id}`)
      .then(res => res.json())
      .then(data => {
        setDetails(data);
  ...
  const handleDelevered = () => {
    ...
    axios.put(
      `https://inventory-update-server.vercel.app/inventory/${id}`, {
      quantity
    })
      .then(response => {
        setReload(!reload);
        setLoading(false);
        toast.success("Product is delevered and decrease quantity !")
      })
      .catch(() => setLoading(false));
  };
  ...
  const handleAddQuantity = async (event) => {
    ...
    await axios.put(`https://inventory-update-server.vercel.app/inventory/${id}`, {
      quantity,
    })
      .then(response => {
        setReload(!reload);
        setLoading(false);
        toast.success("Product quantity aded succesfully!")
      })
      .catch(() => setLoading(false));
  };
  ...
  if (loading || loadingUser) {
   ...
   const displayNamePlaceholder = user.displayName || "Name";
   const emailPlaceholder = user.email || "Email";
            ...
            <Card.Body>
              ...
              <div className='d-flex align-items-center justify-content-between my-2 mb-4'>
                <h5 className="details-price">
                  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="#42baf4" className="bi bi-currency-dollar me-3" viewBox="0 0 16 16">
                ...
                <h6 className="details-quantity">
                  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="#42baf4" className="bi bi-plus-circle-fill me-5" viewBox="0 0 16 16">
          ...
          <div className="form-background">
            <Form onSubmit={handleAddQuantity}>
              <input
                ...
                readOnly
              />
              <input className="input-feild shadow input-feild-2"
                type="email"
